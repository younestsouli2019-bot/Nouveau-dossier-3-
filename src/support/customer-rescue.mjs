...
export async function runCustomerRescue() {
    ...
    console.log(`[Rescue] Found ${affectedCustomers.length} customers needing access.`);

    for (const customer of affectedCustomers) {
        console.log(`[Rescue] Processing ${customer.email} (Course: ${customer.courseId})...`);
        
        try {
            await sendEmergencyAccessEmail(customer);
            console.log(`[Rescue] SUCCESS: Emergency Access sent to ${customer.email}`);
        } catch (e) {
            console.error(`[Rescue] FAILED: Could not reach ${customer.email} - ${e.message}`);
        }
    }
}
...
async function fetchPaidCustomers() {
    ...
    return [
        ...
        { email: "student2@example.com", courseId: "modern-macho-mindset", name: "Bob" }
    ];
}
...
async function sendEmergencyAccessEmail(customer) {
    ...
    console.log(`[CustomerRescue] Triggering Fidelity Journey for ${customer.email}...`);
    await triggerFidelityJourney({ email: customer.email, name: customer.name, product: customer.courseId, purchaseDate: new Date().toISOString() });
}
