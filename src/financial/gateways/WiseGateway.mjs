...
export class WiseGateway {
  ...
  async _ensureProfileAndRecipient() {
    ...
    if (!recipient) throw new Error('WiseGateway: No suitable recipients found')
  ...
  async executeTransfer(transferDetails) {
    ...
    if (
      ...
      for (const t of list) {
        ...
        if (c !== currency) {
          throw new Error(`WiseGateway: Mixed currencies not supported (${c} vs ${currency})`)
        }
    ...
    if (!recipient) throw new Error('WiseGateway: Missing OWNER_WISE_RECIPIENT_ID')
