...
async function forceFlush() {
    ...
    if (pendingItems.length === 0) {
        console.log("No pending items found in ledger scan. (If you expect items, ensure they are in 'payout_items' entity)");
        // We will try to trigger the 'emit-revenue-events' logic which does the scanning
    }

    // 2. Process Items
    for (const item of pendingItems) {
        console.log(`Processing Item ${item.id} (${item.amount} ${item.currency})...`);
        ...
        try {
            ...
            if (!valid) {
                console.error(`!!! SECURITY BLOCK: Item ${item.id} has invalid destination ${resolvedDest}`);
                continue;
            }
