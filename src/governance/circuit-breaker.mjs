...
export async function checkCircuitBreaker(state) {
    ...
    if (spend > 50 && revenue === 0) {
        console.error(`[CIRCUIT BREAKER] TRIPPED! High Spend ($${spend}) with ZERO Revenue. PAUSING SWARM.`);
    ...
    if (revenue > 0 && revenue < 100) {
        console.log(`[CIRCUIT BREAKER] WARNING: Revenue Low ($${revenue}), but running to pay LearnWorlds Bill.`);
    }

    // 2. Error Guard
    if (state.consecutiveFailures > 100) {
        console.error(`[CIRCUIT BREAKER] TRIPPED! Too many errors (${state.consecutiveFailures}). PAUSING SWARM.`);
        state.freeze.active = true;
