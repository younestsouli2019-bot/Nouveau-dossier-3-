...
export async function acquireWorkLease(
	...
	const expiresAtIso = new Date(
		nowMs + Math.max(1000, Number(ttlMs) || 60_000),
	).toISOString();
	...
	if (!existing) {
		try {
			...
			const raced = await findExisting(entity, { [map.key]: k }).catch(
				() => null,
			);
			...
			if (racedExpires != null && racedExpires > nowMs && racedHolder !== h) {
				return {
					acquired: false,
					id: raced.id,
					expiresAt: raced[map.expiresAt] ?? null,
					holder: racedHolder,
				};
			}
			return {
				acquired: racedHolder === h,
				id: raced.id,
				expiresAt: raced[map.expiresAt] ?? null,
				holder: racedHolder,
			};
	...
	if (!isExpired && existingHolder !== h) {
		return {
			acquired: false,
			id: existing.id,
			expiresAt: existing[map.expiresAt] ?? null,
