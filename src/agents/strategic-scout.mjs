...
class StrategicScout {
	...
	async loadHeuristics() {
		const ethicalPath = path.resolve(
			process.cwd(),
			"src/agents/config/ethical-heuristics.json",
		);
		const technicalPath = path.resolve(
			process.cwd(),
			"src/agents/config/technical-heuristics.json",
		);
	 ...
	 */
		...
		if (proposal && typeof proposal === "object") {
			if (proposal.category) {
				sectors.push(String(proposal.category).toLowerCase());
			}
			if (proposal.domain) {
				sectors.push(String(proposal.domain).toLowerCase());
			}
			if (Array.isArray(proposal.tags)) {
				for (const t of proposal.tags) {
					sectors.push(String(t).toLowerCase());
				}
			}
		}

		const bannedHit = bannedSectors.find((s) =>
			sectors.some((sec) => sec.includes(s)),
		);
		const hardBanHit = hardBanKeywords.find((kw) =>
			proposalText.includes(kw),
		);

		if (bannedHit || hardBanHit) {
			const label = bannedHit || hardBanHit;
			analysis.summary.push(
				`CRITICAL FAIL: Proposal violates owner ethics for '${label}'.`,
			);
