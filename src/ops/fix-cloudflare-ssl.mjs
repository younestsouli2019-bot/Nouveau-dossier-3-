...
export async function fixCloudflareSsl() {
    ...
    if (!token || !zoneId) {
        console.error("❌ MISSING CREDENTIALS: CLOUDFLARE_API_TOKEN or CLOUDFLARE_ZONE_ID not found in .env");
        console.log("   -> Cannot automate fix. Please manually set SSL to 'Full (Strict)' in Cloudflare Dashboard.");
        return;
    }
        ...
        const req = https.request({
            ...
            res.on("end", () => {
                if (res.statusCode === 200) {
                    ...
                    console.error(`[Cloudflare] FAILED: '${settingName}' - Status ${res.statusCode}`);
