...
export async function POST(request: NextRequest) {
  try {
    ...
    for (const batch of batches) {
      ...
      if (!provider || !['paypal', 'payoneer', 'bank_transfer', 'attijari'].includes(provider)) {
        console.error(
      ...
      const recipients: PayoutRecipient[] = batch.items.map((item) => ({
        ...
        accountId: item.paymentMethod === 'bank_transfer' || provider === 'attijari' ? item.transactionRef || undefined : undefined,
