...
interface ProcurementItem {
  ...
  notes: string | null; priority: string; purpose: string | null; fulfillmentSource: string | null
...
export default function SupplyChainDashboard() {
  ...
  const handleVerifyAll = async () => {
    ...
    try {
      ...
      setProgressLog(prev => [{ msg: d.message, time: new Date().toLocaleTimeString(), type: d.verified > 0 ? ('verify' as const) : ('info' as const) }, ...prev].slice(0, 30))
      fetchData()
  ...
  const handleAdvanceProgress = async (steps = 1) => {
    ...
    try {
      ...
      toast.success(d.message)
      setProgressLog(prev => [{ msg: d.message, time: new Date().toLocaleTimeString(), type: d.delivered > 0 ? ('success' as const) : ('advance' as const) }, ...prev].slice(0, 30))
      if (d.details) {
    ...
    <div className="min-h-screen flex flex-col bg-background">
      ...
      <footer className="border-t bg-card/50 py-3 mt-auto">
        <div className="max-w-[1600px] mx-auto px-4 sm:px-6 flex flex-col sm:flex-row items-center justify-between gap-1 text-xs text-muted-foreground">
          <span>Supply Chain Management — Younes Tsouli CIN:[REDACTED]</span>
