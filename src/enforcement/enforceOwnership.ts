...
export async function enforceOwnership(transaction: any): Promise<boolean> {
    ...
    if (!validateFlow(src, dest)) {
        await freezeSystem(`Enforcement: INVALID FLOW. Destination [${dest}] is NOT a valid Owner Account or Internal Ops Account.`);
        return false;
    }

    // Evidence Integrity Chain check (Stub)
    if (!transaction.evidenceChain || !transaction.evidenceChain.valid) {
        console.warn("Enforcement: Evidence chain missing or invalid (Warning only for now)");
    }
