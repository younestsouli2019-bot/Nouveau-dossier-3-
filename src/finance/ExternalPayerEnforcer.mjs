...
export class ExternalPayerEnforcer {
	...
	async runEnforcementCycle() {
		...
		const pending = allEvents.filter(
			(e) => e.status === "settled_externally_pending",
		);

		const collections = [];

		for (const payer of payers) {
			const payerEvents = pending.filter(
				(e) => e.metadata?.payer_email === payer.email,
			);
			...
			payerEvents.forEach((e) => {
				...
				if (!invoices[ref])
					invoices[ref] = { count: 0, amount: 0, oldest: e.created_date };
				invoices[ref].count++;
			...
			if (priorityDebtors.includes(payer.email)) {
				console.warn(
					`[Enforcer] 🚨 PRIORITY DEBTOR DETECTED: ${payer.name} (${payer.email}) has outstanding balance.`,
				);
			}
		...
		if (collections.length > 0) {
			console.warn(
				`[Enforcer] 🛑 ${collections.length} payers are overdue. Triggering STOP WORK protocols.`,
			);
