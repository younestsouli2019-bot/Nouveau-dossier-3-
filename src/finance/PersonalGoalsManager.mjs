...
export class PersonalGoalsManager {
    ...
    async allocate(amount) {
        ...
        const salaryShare = amount * 0.40;
        this.state.allocated["salary"] = (this.state.allocated["salary"] || 0) + salaryShare;
        allocationReport.push({ target: "Monthly Salary", amount: salaryShare });
        remaining -= salaryShare;

        // 2. Debt / Essentials (40%)
        const debtShare = amount * 0.40;
        this.state.allocated["debt"] = (this.state.allocated["debt"] || 0) + debtShare;
        allocationReport.push({ target: "Debt Clearance", amount: debtShare });
        remaining -= debtShare;

        // 3. Specific Assets (20% + Remainder)
        // Sort goals by priority
        const activeGoals = this.goals.goals.sort((a, b) => a.priority - b.priority);
        ...
        for (const goal of activeGoals) {
            ...
            remaining -= toAdd;
            allocationReport.push({ target: goal.name, amount: toAdd, progress: ((current + toAdd) / goal.target * 100).toFixed(1) + "%" });
        }

        // If money is left over, dump into Savings
        if (remaining > 0) {
            this.state.allocated["savings"] = (this.state.allocated["savings"] || 0) + remaining;
    ...
    async getProgressReport() {
        ...
        report.push(`💵 Salary: $${salaryCurrent.toFixed(2)} / $${salaryTarget} (${(salaryCurrent/salaryTarget*100).toFixed(1)}%)`);
        ...
        for (const goal of this.goals.goals) {
            ...
            const status = curr >= goal.target ? "✅ COMPLETED" : `${(curr/goal.target*100).toFixed(1)}%`;
            report.push(`🎯 ${goal.name}: $${curr.toFixed(2)} / $${goal.target} [${status}]`);
        }
