...
function resolveServerUrl() {
	const explicit =
		process.env.BASE44_API_URL || process.env.BASE44_SERVER_URL || null;
	const normalized = normalizeUrlBase(explicit);
	return normalized || "https://api.base44.app";
}
...
function createOnlineClient() {
	if (!createClient) {
		throw new Error("Base44 SDK unavailable");
	}
...
function getOnlineAuth() {
	...
	const envServiceToken = coerceNonEmptyString(
	...
	if (envAppId && envServiceToken) {
		...
		if (decoded) {
			const tokenAppId = normalizeAppIdInput(
				decoded.appId ?? decoded.app_id ?? decoded.applicationId,
			);
			if (tokenAppId && !appIdEquivalent(tokenAppId, envAppId)) {
				throw new Error(
					`Security Mismatch: BASE44_APP_ID (${envAppId}) does not match token's app_id (${tokenAppId})`,
				);
			}
		}
	...
	const appId =
		envAppId ??
		parsed.appId ??
		(serviceToken ? inferAppIdFromServiceToken(serviceToken) : null);

	if (!appId) throw new Error("Missing required env var: BASE44_APP_ID");
	if (!serviceToken)
		throw new Error("Missing required env var: BASE44_SERVICE_TOKEN");
	...
	if (serviceToken) {
		...
		if (decoded) {
			...
			if (tokenAppId && !appIdEquivalent(tokenAppId, appId)) {
				throw new Error(
					`Security Mismatch: Inferred App ID (${appId}) does not match token's app_id (${tokenAppId})`,
				);
			}
		}
	}
...
export function buildBase44Client({
	...
	// 1. Force Local Mode if SWARM_MODE is local (Highest Priority)
    if (process.env.SWARM_MODE === 'local') {
        console.log("[Base44Client] SWARM_MODE=local detected. Forcing Offline Client.");
    ...
    // 2. Wrap Online Client with Auto-Fallback
    try {
        return createOnlineClient();
    } catch (e) {
        console.warn(`[Base44Client] Online client creation failed: ${e.message}. Falling back to Offline/Local Store.`);
        return createOfflineClient({ filePath: getOfflineStorePath() });
    }
}
