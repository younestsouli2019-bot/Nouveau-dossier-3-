...
export function getPublicKeyFromEnvKid(kid) {
	...
	if (directJson) {
		...
		if (pem)
			return crypto.createPublicKey(
				parseKeyLike(pem, `AP2_PUBLIC_KEYS_JSON[${k}]`),
			);
	}
	...
	if (!pem)
		throw new Error(
			`Missing public key for kid=${k} (set ${envKey} or AP2_PUBLIC_KEYS_JSON)`,
		);
	return crypto.createPublicKey(parseKeyLike(pem, envKey));
}

export function signMandatePayload(
	payload,
	{ kid, privateKey = null, protectedHeader = {} } = {},
) {
	const k = String(kid ?? "").trim();
...
export function verifyMandateEnvelope(
	...
	if (!protectedHeader || typeof protectedHeader !== "object")
		violations.push("missing_protected");
	if (!payload || typeof payload !== "object")
		violations.push("missing_payload");
	...
	if (protectedHeader?.alg && protectedHeader.alg !== "Ed25519")
		violations.push("unsupported_alg");
...
export function buildMandateChainHash(prevEnvelope) {
	...
	if (!payload || typeof payload !== "object")
		throw new Error("Invalid previous mandate");
