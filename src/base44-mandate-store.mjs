...
function buildMandateData(
	...
	if (!payload || typeof payload !== "object")
		throw new Error("Mandate payload is required");
	if (!protectedHeader || typeof protectedHeader !== "object")
		throw new Error("Mandate protected header is required");
	if (!signature || typeof signature !== "string")
		throw new Error("Mandate signature is required");
...
export async function writeBase44MandateIdempotent(
	...
	try {
		...
		const racedId = await findExistingId(entity, primaryFilter).catch(
			() => null,
		);
