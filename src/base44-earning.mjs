...
export function getEarningConfigFromEnv() {
	const entityName = process.env.BASE44_EARNING_ENTITY ?? "earnings";
...
function buildEarningData(cfg, earning) {
	...
	if (!amount || Number.isNaN(amount) || amount <= 0)
		throw new Error("Earning amount must be > 0");

	const currency = earning.currency ?? cfg.defaultCurrency;
	const occurredAt =
		earning.occurredAt ?? earning.occurred_at ?? new Date().toISOString();
	...
	if (earning.settlementId != null)
		data[map.settlementId] = earning.settlementId;
	data[map.metadata] = earning.metadata ?? {};

	return data;
}

export async function createBase44EarningIdempotent(
	base44,
	cfg,
	earning,
	{ dryRun = false } = {},
	...
	const enable =
		(process.env.BASE44_ENABLE_EARNING_WRITES ?? "false").toLowerCase() ===
		"true";
	if (!enable) {
		throw new Error(
			"Refusing to create earnings without BASE44_ENABLE_EARNING_WRITES=true",
		);
	}

	const entity = base44.asServiceRole.entities[cfg.entityName];
	const existingId = await findExistingId(entity, {
		[cfg.fieldMap.earningId]: data[cfg.fieldMap.earningId],
	});
