...
export function checkNewBatches({
	dir = "settlements/payoneer",
	notifyDir = "data/notifications",
} = {}) {
	...
	if (news.length) {
		const outFile = path.resolve(
			notifyDir,
			`new-payoneer-batches_${Date.now()}.json`,
		);
		ensureDir(path.dirname(outFile));
		fs.writeFileSync(
			outFile,
			JSON.stringify({ created_at: now, files: news }, null, 2),
		);
