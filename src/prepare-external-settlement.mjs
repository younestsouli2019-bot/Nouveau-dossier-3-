...
function getBankDetails(args) {
	...
	const get = (name, fallbackEnv = null) =>
		fromArgs(name) ?? process.env[name] ?? (fallbackEnv ? process.env[fallbackEnv] ?? null : null);

	const beneficiaryName = get("BANK_BENEFICIARY_NAME", "OWNER_BENEFICIARY_NAME");
	const bankName = get("BANK_NAME");
	const iban = get("BANK_IBAN", "OWNER_IBAN");
	const rib = get("BANK_RIB", "MOROCCAN_BANK_RIB");
	const swift = get("BANK_SWIFT", "OWNER_SWIFT");
