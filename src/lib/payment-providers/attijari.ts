...
export function getAttijariConfig(): AttijariApiConfig | null {
 const cfg = loadConfigJson()
  ...
  return {
    baseUrl: baseUrl.replace(/\/+$/, ''), clientId, clientSecret, oauthTokenUrl: cfg.api.auth.oauthTokenUrl, scope: cfg.api.auth.scope, timeouts: cfg.api.timeouts,
    enabled: process.env.ATTIJARI_ENABLED ? process.env.ATTIJARI_ENABLED === 'true' : true,
    // SAFETY: sandbox defaults to true so no real funds move unless explicitly opted in
    sandbox: process.env.ATTIJARI_SANDBOX ? process.env.ATTIJARI_SANDBOX !== 'false' : true,
  }
}
...
export function generateAttijariPain001(recipients: PayoutRecipient[], batchRef: string, debitRib: string = '[REDACTED]'): string {
