...
export function getBankWireConfig(): BankWireConfig | null {
  const bankName = process.env.BANK_WIRE_BANK_NAME || process.env.BANK_NAME_BC;
  const bic = process.env.BANK_WIRE_BIC || process.env.BIC_BC;
  const accountNumber = process.env.BANK_WIRE_ACCOUNT_NUMBER || process.env.IBAN_BC;
  const accountName = process.env.BANK_WIRE_ACCOUNT_NAME || process.env.BENEFICIARY_NAME_BC;
  const currency = process.env.BANK_WIRE_CURRENCY || 'EUR';
  ...
  const sandbox = process.env.ENABLE_REAL_MONEY_MOVEMENT !== 'true' && process.env.BANK_SANDBOX !== 'false';
...
async function submitViaApi(
  ...
  try {
    console.log(
      `[BankWire] Submitting via API: ${batchReference} with ${recipients.length} items to ${config.apiEndpoint}`,
    );
