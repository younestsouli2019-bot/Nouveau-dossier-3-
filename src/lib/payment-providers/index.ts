...
import {
  getAttijariConfig,
  submitAttijariPayout,
  testAttijariConnection,
  healthCheck as attijariHealthCheck,
} from './attijari';
import type { AttijariApiConfig } from './attijari';

export type PaymentProvider = 'paypal' | 'payoneer' | 'bank_transfer' | 'attijari';
...
export async function submitPayoutToProvider(
  ...
  switch (provider) {
    ...
    case 'attijari': {
      const config = getAttijariConfig();
      if (!config) {
        throw new Error(
          'Attijari is not configured. Set ATTIJARI_CLIENT_ID, ATTIJARI_CLIENT_SECRET, and ATTIJARI_API_BASE_URL in .env',
        );
      }
      if (!config.enabled) {
        throw new Error('Attijari is disabled in configuration');
      }
      return submitAttijariPayout(config, recipients, batchReference);
    }
  ...
  switch (provider) {
    ...
    case 'attijari': {
      ...
      if (!config) {
        return {
          connected: false,
          error: 'Attijari is not configured. Set ATTIJARI_CLIENT_ID, ATTIJARI_CLIENT_SECRET, and ATTIJARI_API_BASE_URL in .env',
        };
      }
      return testAttijariConnection(config);
    }
...
export function getProviderConfig(
  provider: PaymentProvider,
): PayPalConfig | PayoneerConfig | BankWireConfig | AttijariApiConfig | null {
  switch (provider) {
    case 'paypal':
...
/**
 * Run health checks on all payment providers concurrently.
...
export async function healthCheckAll(): Promise<{
  ...
  attijari: HealthCheckResult;
  anyAvailable: boolean;
}> {
  console.log(`[PaymentProvider] healthCheckAll: starting concurrent health checks`);

  const [paypalResult, payoneerResult, bankTransferResult, attijariResult] = await Promise.all([
    ...
    (async (): Promise<HealthCheckResult> => {
      ...
      if (!config) {
        return {
          available: false,
          latencyMs: 0,
          details: { reason: 'not_configured', sandboxDefault: true },
          error: 'Attijari is not configured. Set ATTIJARI_CLIENT_ID, ATTIJARI_CLIENT_SECRET, and ATTIJARI_API_BASE_URL in .env',
        };
      }
      const result = await attijariHealthCheck(config);
      return {
        available: result.available,
        latencyMs: result.latencyMs,
        details: { ...(result.details as Record<string, unknown>), sandbox: config.sandbox, enabled: config.enabled },
        error: result.error,
      };
    })(),
  ]);

  const anyAvailable = paypalResult.available || payoneerResult.available || bankTransferResult.available || attijariResult.available;

  console.log(
    `[PaymentProvider] healthCheckAll: complete — paypal=${paypalResult.available}, payoneer=${payoneerResult.available}, bankTransfer=${bankTransferResult.available}, attijari=${attijariResult.available}, anyAvailable=${anyAvailable}`,
  );
...
export type { AttijariApiConfig } from './attijari';