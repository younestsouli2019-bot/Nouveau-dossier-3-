...
export async function triggerFidelityJourney(customer) {
    console.log(`[Fidelity] MANUAL TRIGGER: Starting Journey for ${customer.email}`);
...
export async function runFidelityLoop() {
    ...
    for (const customer of customers) {
        ...
        try {
            ...
            console.error(`[Fidelity] Failed to nurture ${customer.email}: ${e.message}`);
        }
    }
}

async function sendWelcome(c) {
    console.log(`[Fidelity] Day 0: Sending VIP Welcome to ${c.name}. "You made the right choice."`);
    // Logic: Send email with direct login + unadvertised bonus
}

async function sendCheckIn(c) {
    console.log(`[Fidelity] Day 3: Checking in on ${c.name}. "Stuck? Let me help."`);
    // Logic: Trigger "CustomerRescue" if they haven't logged in
}

async function sendValueAdd(c) {
    console.log(`[Fidelity] Day 7: Sending Free Gift to ${c.name}. "Thinking of you."`);
...
async function proposeAscension(c) {
    console.log(`[Fidelity] Day 30: Offering Mentorship to ${c.name}. "Ready for the next level?"`);
}
...
async function fetchCustomers() {
    ...
    return [
        { name: "New Guy", email: "new@test.com", purchaseDate: new Date().toISOString() },
        { name: "Week Old", email: "week@test.com", purchaseDate: new Date(Date.now() - 7 * 86400000).toISOString() }
    ];
}
