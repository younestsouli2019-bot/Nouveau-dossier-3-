...
export async function recordAudit(action, payload) {
	...
	const { payload: safePayload, mutated } = guardrails.injectEntropyIntoAudit(
		action,
		payload ?? {},
	);
	...
	const entry = JSON.stringify({ at, action, payload: safePayload }) + "\n";
	await fs.appendFile(file, entry, "utf8").catch(async () => {
		await fs.writeFile(file, entry, "utf8");
	});
	return { ok: true, at, file, mutated };
}
