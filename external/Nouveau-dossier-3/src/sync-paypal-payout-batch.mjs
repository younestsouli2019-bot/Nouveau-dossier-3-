...
function shouldWriteToBase44() {
	return (
		(
			process.env.BASE44_ENABLE_PAYPAL_PAYOUT_SYNC_WRITE ?? "false"
		).toLowerCase() === "true"
	);
}
...
function getEntityConfig() {
	return {
		...
		fieldMap: {
			...
			processedAt:
				process.env.BASE44_PAYOUT_BATCH_FIELD_PROCESSED_AT ?? "processed_at",
...
async function upsertBatch(base44, cfg, details) {
	...
	const processedAt =
		...
		details?.batch_header?.time_created ??
		null;
...
async function main() {
	...
	if (!shouldWriteToBase44()) {
		process.stdout.write(
			`${JSON.stringify({ ok: true, stored: false, details })}\n`,
		);
		return;
	}
