...
export function getExternalSettlementConfigFromEnv() {
	const entityName =
		process.env.BASE44_EXTERNAL_SETTLEMENT_ENTITY ?? "ExternalSettlement";
	const mapFromEnv = safeJsonParse(
		process.env.BASE44_EXTERNAL_SETTLEMENT_FIELD_MAP,
		null,
	);
...
function buildSettlementData(cfg, settlement) {
	...
	const settlementId =
		settlement.settlementId ?? settlement.settlement_id ?? null;
	if (!settlementId)
		throw new Error("External settlement requires settlementId");

	const amount = Number(settlement.amount);
	if (!amount || Number.isNaN(amount) || amount <= 0)
		throw new Error("External settlement amount must be > 0");

	return {
		[map.settlementId]: String(settlementId),
		[map.periodStart]:
			settlement.periodStart ?? settlement.period_start ?? null,
		...
		[map.referenceId]:
			settlement.referenceId ?? settlement.reference_id ?? null,
...
export async function createBase44ExternalSettlementIdempotent(
	...
	const enable =
		(
			process.env.BASE44_ENABLE_EXTERNAL_SETTLEMENT_WRITES ?? "false"
		).toLowerCase() === "true";
	if (!enable) {
		throw new Error(
			"Refusing to create external settlements without BASE44_ENABLE_EXTERNAL_SETTLEMENT_WRITES=true",
		);
	}

	const entity = base44.asServiceRole.entities[cfg.entityName];
	const existingId = await findExistingId(entity, {
		[cfg.fieldMap.settlementId]: data[cfg.fieldMap.settlementId],
	});
	...
	try {
		...
		const racedId = await findExistingId(entity, {
			[cfg.fieldMap.settlementId]: data[cfg.fieldMap.settlementId],
		}).catch(() => null);
