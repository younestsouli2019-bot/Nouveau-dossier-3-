...
async function executePayPalSettlement(batch) {
	...
	const itemsInLedger = await itemEntity
		.filter({ batch_id: String(batch.batch_id) }, "-created_date", 500, 0)
		.catch(() => []);
...
async function executeCryptoSettlement(batch) {
	const allowMicro =
		String(process.env.ALLOW_MICROPAYMENTS_TO_OWNER ?? "true").toLowerCase() ===
		"true";
	...
	if (payload.length === 0 && allowMicro) {
		const events = batch.items
			.map((x) => ({
				id: x?.revenue_event_id ?? null,
				amount: Math.max(minMicro, Number(x?.amount ?? 0)),
	...
	const base44 = buildBase44ServiceClient();
	const batchEntity = base44.asServiceRole.entities["PayoutBatch"];
	const recs = await batchEntity
		.filter({ batch_id: String(batch.batch_id) }, "-created_date", 1, 0)
		.catch(() => []);
	const id = Array.isArray(recs) && recs[0]?.id ? recs[0].id : null;
	if (id) {
		const notes = {
			...(recs[0]?.notes ?? {}),
			crypto_export_ready: true,
		...
		await batchEntity
			.update(id, { status: "export_ready_crypto", notes })
			.catch(() => null);
	}
	return { ok: true, crypto_export_ready: payload.length };
}
...
async function markEventsSettled(eventIds, batchId) {
	...
	for (const id of eventIds) {
		const recs = await entity
			.filter({ [cfg.fieldMap.externalId]: String(id) }, "-created_date", 1, 0)
			.catch(() => []);
		...
		if (cfg.fieldMap.payoutBatchId)
			patch[cfg.fieldMap.payoutBatchId] = String(batchId);
...
function getOwnerAccounts() {
	return {
		...
		bank: {
			rib: String(process.env.OWNER_BANK_RIB ?? "007810000448500030594182"),
		},
		payoneer: {
			accountId: String(
				process.env.OWNER_PAYONEER_ACCOUNT_ID ?? "PRINCIPAL_ACCOUNT",
			),
		},
	};
}
...
function shouldWritePayoutLedger() {
	return (
		String(
			process.env.BASE44_ENABLE_PAYOUT_LEDGER_WRITE ?? "false",
		).toLowerCase() === "true"
	);
}
...
function getRoutingPriority() {
	...
	return raw
		.split(/[|,; ]/g)
		.map((x) => map[String(x).toLowerCase()] || "")
		.filter(Boolean);
}
...
async function executeApprovedBatchesSunday() {
	...
	const base44 = buildBase44ServiceClient();
	const batchEntity = base44.asServiceRole.entities["PayoutBatch"];
	const batches = await batchEntity
		.filter({ status: "approved" }, "-created_date", 250, 0)
		.catch(() => []);
...
async function loadBatchItems(base44, batchId) {
	const itemEntity = base44.asServiceRole.entities["PayoutItem"];
	const list = await itemEntity
		.filter({ batch_id: String(batchId) }, "-created_date", 500, 0)
		.catch(() => []);
...
async function retryAndRerouteFailedPayouts() {
	...
	const batchEntity = base44.asServiceRole.entities["PayoutBatch"];
	const itemEntity = base44.asServiceRole.entities["PayoutItem"];
	const failed = await itemEntity
		.filter({ status: "failed" }, "-created_date", 500, 0)
		.catch(() => []);
	...
	for (const [batchId, items] of grouped.entries()) {
		const allowMicro =
			String(
				process.env.ALLOW_MICROPAYMENTS_TO_OWNER ?? "true",
			).toLowerCase() === "true";
		const minMicro = Number(process.env.MIN_MICRO_AMOUNT_USD ?? "0.01");
		const next = allowMicro ? "PAYPAL" : "PAYONEER";
		let original = null;
		try {
			const list = await batchEntity.filter(
				{ batch_id: String(batchId) },
				"-created_date",
				1,
				0,
		...
		await processRailBatch(next, events, {
			microReroute: allowMicro && next === "PAYPAL",
		}).catch(() => {});
		if (original?.id) {
			const notes = {
				...(original?.notes ?? {}),
				micro_reroute: allowMicro && next === "PAYPAL",
				rerouted_to: next,
