...
import { sha256, isCryptographicallyVerifiable, computeSettlementIntegrityHash } from './crypto-utils'
...
export async function strictMarkAsSettled(params: {
    ...
    if (proofHash) {
      ...
      if (computedHash !== proofHash) {
        return {
          success: false,
          settlementId,
          error: 'STRICT VIOLATION: proofHash does not match the computed hash of settlementId + externalRef + amount + currency.',
  ...
  const updated = await db.ownerSettlement.update({
    ...
    data: {
      ...
      settledAt: newStatus === 'completed' ? new Date() : existing.settledAt,
    },
  })
  ...
  // TRUTH-SETTLE: Compute settlement integrity hash for the owner account
  if (newStatus === 'completed' && externalRef) {
    try {
      const accountSettlements = await db.ownerSettlement.findMany({
        where: {
          ownerAccountId: existing.ownerAccountId,
          status: 'completed',
        },
        select: { id: true, amount: true, currency: true, externalRef: true },
      })
      const integrityHash = computeSettlementIntegrityHash(accountSettlements)
      // Store the batch integrity hash on the account level via metadata
      const accountMeta = await db.ownerAccount.findUnique({
        where: { id: existing.ownerAccountId },
        select: { notes: true },
      })
      const metaEntry = JSON.stringify({
        lastSettlementIntegrityHash: integrityHash,
        lastVerifiedAt: new Date().toISOString(),
        settlementCount: accountSettlements.length,
      })
      await db.ownerAccount.update({
        where: { id: existing.ownerAccountId },
        data: {
          notes: accountMeta?.notes
            ? `${accountMeta.notes} | [TRUTH-INTEGRITY] ${metaEntry}`
            : `[TRUTH-INTEGRITY] ${metaEntry}`,
        },
      })
    } catch (e) {
      console.error('[StrictSettlement] Integrity hash computation failed (non-blocking):', e)
    }
  }
