...
export const RECIPIENT_ADDRESSES: Record<string, { address: string; city: string }> = {
  hind: {
    address: 'Etage 2 JASMIN II IMM H3 APPT 21, Sidi Yahya Zair, Kenitra 12150, Morocco',
    city: 'Kenitra',
  },
  younes: {
    address: 'Lot. Rita LOT C Im B, APT 17, Bouznika, Casablanca-Settat 13100, Morocco',
    city: 'Bouznika',
  },
  bachir: {
    address: '45 Avenue Ibn Sina, Agdal, Rabat, Morocco',
    city: 'Rabat',
  },
}

export const HIND_ADDRESS = RECIPIENT_ADDRESSES.hind.address
export const YOUNES_ADDRESS = RECIPIENT_ADDRESSES.younes.address
export const BACHIR_ADDRESS = RECIPIENT_ADDRESSES.bachir.address

export function getDestinationInfo(
  recipientName: string,
): { address: string; city: string } {
  const n = (recipientName || '').toLowerCase()
  if (n.includes('hind')) return RECIPIENT_ADDRESSES.hind
  if (n.includes('bachir')) return RECIPIENT_ADDRESSES.bachir
  return RECIPIENT_ADDRESSES.younes
}
...
export async function seedProcurement(
  dbClient: typeof db,
  data: SeedItem[] = SEED_DATA,
): Promise<{ created: number; skipped: number; skippedItems: string[] }> {
  let created = 0
  let skipped = 0
  const skippedItems: string[] = []

  for (const item of data) {
    const existing = await dbClient.procurementItem.findFirst({
      where: {
        name: item.name,
    ...
    const resolved = getDestinationInfo(item.recipientName)
    const recipientAddress = item.recipientAddress || resolved.address

    await dbClient.procurementItem.create({
      data: {
        name: item.name,
