...
import {
	parseCommaList,
	resolveMissionFilters,
	missionMatchesFilters,
} from "../src/prepare-revenue-sweep.mjs";
...
test("missionMatchesFilters matches by id", () => {
	...
	assert.equal(
		missionMatchesFilters({ id: "id3", title: "x" }, filters),
		false,
	);
});

test("missionMatchesFilters matches by title (case-insensitive)", () => {
	const filters = resolveMissionFilters({
		"only-mission-titles": "Mission Atlas-Fund",
	});
	assert.equal(
		missionMatchesFilters({ id: "x", title: "MISSION ATLAS-FUND" }, filters),
		true,
	);
	assert.equal(
		missionMatchesFilters({ id: "x", title: "Other" }, filters),
		false,
	);
});
