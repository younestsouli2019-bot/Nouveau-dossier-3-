...
function main() {
	...
	const name =
		args.recipient_name ||
		process.env.BENEFICIARY_NAME_BC ||
		process.env.BENEFICIARY_NAME_BARCLAYS ||
	...
	const batchId =
		args.batch_id || process.env.PAYONEER_BATCH_ID || nowBatchId();
	const itemId = args.item_id || `${batchId}-ITEM-1`;
	const note = args.note || process.env.SETTLEMENT_NOTE || batchId;
	const payerName =
		args.payer_name || process.env.SETTLEMENT_REQUESTOR_NAME || "";
	const payerEmail =
		args.payer_email || process.env.SETTLEMENT_REQUESTOR_EMAIL || "";
	const payerCompany =
		args.payer_company || process.env.SETTLEMENT_REQUESTOR_COMPANY || "";
	const purpose =
		args.purpose || process.env.SETTLEMENT_PURPOSE || "Service/Settlement";
	const reference =
		args.reference || process.env.SETTLEMENT_REFERENCE || batchId;
	...
	const norm = (s) =>
		String(s || "")
			...
			.toLowerCase();
	if (
		norm(payerEmail) === norm(email) ||
		norm(payerEmail) === norm(recipient)
	) {
