	...
	if (!payload) {
		...
		payload = {
			...
			bank: {
				enabled:
					String(
						args.bank_enable ?? process.env.BANK_WIRE_ENABLE ?? "false",
					).toLowerCase() === "true",
				provider: String(
					args.bank_provider ?? process.env.BANK_WIRE_PROVIDER ?? "",
				).toUpperCase(),
				beneficiaryName:
					args.beneficiary_name ?? process.env.OWNER_BENEFICIARY_NAME ?? "",
				iban:
					args.iban ?? process.env.OWNER_IBAN ?? process.env.BANK_IBAN ?? "",
			...
			payoneer: {
				...
				clientId:
					args.payoneer_client_id ?? process.env.PAYONEER_CLIENT_ID ?? "",
				clientSecret:
					args.payoneer_client_secret ??
			...
			cryptobox: {
				enabled:
					String(
						args.cryptobox_enable ?? process.env.CRYPTOBOX_ENABLE ?? "false",
					).toLowerCase() === "true",
				url:
					args.binance_cryptobox_url ??
			...
			googlepay: {
				enabled:
					String(
						args.googlepay_enable ?? process.env.GOOGLEPAY_ENABLE ?? "false",
					).toLowerCase() === "true",
				email:
