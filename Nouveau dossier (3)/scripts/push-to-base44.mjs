...
class Base44Pusher {
	...
	buildHeaderCandidates() {
		const token = String(this.config.serviceToken || "");
		const appId = String(this.config.appId || "");
		const userAccessToken = String(
			process.env.BASE44_ACCESS_TOKEN ||
				process.env.BASE44_USER_TOKEN ||
				process.env.BASE44_BEARER_TOKEN ||
				"",
		).trim();
		const sessionCookie = String(
			process.env.BASE44_SESSION_COOKIE || process.env.BASE44_COOKIE || "",
		).trim();
		const common = {
		...
		variants.push(withCookie({ ...common, "X-API-Key": token, "X-App-Id": appId }));
		variants.push(
			withCookie({ ...common, "X-Service-Token": token, "X-App-Id": appId }),
		);
		variants.push(withCookie({ ...common, Authorization: `Bearer ${token}` }));
		variants.push(withCookie({ ...common, "X-API-Key": token }));
		variants.push(
			withCookie({
				...common,
				"X-Base44-Service-Token": token,
				"X-Base44-App-Id": appId,
			}),
		);
		variants.push(
			withCookie({
	...
	async request(endpoint, method = "GET", body = null) {
		...
		const headerCandidates = this.buildHeaderCandidates();
		let lastErr = null;
		for (const headers of headerCandidates) {
			...
			try {
				...
				lastErr = error;
				this.log(`Request failed: ${error.message}`, "warning");
				continue;
			}
		}
		this.log(`Request failed for all header variants`, "error");
		throw lastErr || new Error("Base44 request failed");
	}
