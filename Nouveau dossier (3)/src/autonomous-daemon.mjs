...
function enforceSwarmLiveHardInvariant({ component, action }) {
	...
	if (
		...
		throw new Error(
			"FATAL: Simulation/Mock flags detected. System halted to prevent phantom data.",
		);
	}
...
async function runTick(cfg, state) {
	...
	if (state.tickCount % 100 === 0) {
		...
		if (status.totalDebt > 0) {
			console.log(
				`[Daemon] DEBT ALERT: Net Net Position is NEGATIVE (-${status.formatted}). Revenue Priority: CRITICAL.`,
			);
		}
	}
	...
	if (state.tickCount % 5 === 0) {
		await runCustomerRescue().catch((err) =>
			console.error("[Daemon] Customer Rescue Error:", err),
		);
	}

	// FIDELITY & RETENTION (Daily)
	if (state.tickCount % 100 === 0) {
		await runFidelityManager().catch((err) =>
			console.error("[Daemon] Fidelity Error:", err),
		);
	}
	...
	if (state.tickCount % 10 === 0) {
		await enforceCovenant(state).catch((err) =>
			console.error("[Daemon] Covenant Enforcement Error:", err),
		);
	}

	// SELF-HEALING: Wake up Zombie Agents
	if (state.tickCount % 50 === 0) {
		await runSelfHealing(state).catch((err) =>
			console.error("[Daemon] Self-Healing Error:", err),
		);
	}
	...
	if (state.tickCount % 10 === 0) {
		await runProfitFeedbackLoop().catch((err) =>
			console.error("[Daemon] Feedback Loop Error:", err),
		);
	}

	// HUNTER-GATHERER LOOP (Insight -> Action)
	if (state.tickCount % 5 === 0) {
		await runHunterGathererLoop().catch((err) =>
			console.error("[Daemon] Hunter Loop Error:", err),
		);
	}
	...
	if (state.tickCount % 20 === 0) {
		await runUstadhAgent().catch((err) =>
			console.error("[Daemon] Ustadh Error:", err),
		);
	}
	...
	if (scaling.forceRun.length > 0) {
		console.log(
			`[Daemon] Swarm Scaler triggered: ${scaling.forceRun.join(", ")}`,
		);
