...
export class OwnerSettlementEnforcer {
	static getPaymentConfiguration() {
		const priorityEnv =
			...
			"bank_transfer,payoneer,crypto,paypal,googlepay";
		let settlement_priority = priorityEnv
			.split(",")
			.map((r) => r.trim())
			.filter(Boolean);
			...
			if (fs.existsSync(p)) {
				...
				if (j && typeof j === "object") {
					...
					if (Array.isArray(j.priority) && j.priority.length) {
						settlement_priority = j.priority
							.map((r) => String(r).trim())
							.filter(Boolean);
					}
				}
		...
		const creds = {
			...
			attijari_morocco: {
				enabled:
					String(process.env.ATTIJARI_ENABLE || "false").toLowerCase() ===
					"true",
				rib: process.env.ATTIJARI_RIB,
			},
			chimoney: {
				enabled:
					String(process.env.CHIMONEY_ENABLE || "false").toLowerCase() ===
					"true",
				id: process.env.CHIMONEY_ID,
			},
			xe: {
				enabled:
					String(process.env.XE_ENABLE || "false").toLowerCase() === "true",
			...
			transfi: {
				enabled:
					String(process.env.TRANSFI_ENABLE || "false").toLowerCase() ===
					"true",
		...
		if (json && typeof json === "object") {
			if (json.paypal && typeof json.paypal === "object")
				creds.paypal = { ...creds.paypal, ...json.paypal };
			if (json.bank && typeof json.bank === "object")
				creds.bank = { ...creds.bank, ...json.bank };
			if (json.payoneer && typeof json.payoneer === "object")
				creds.payoneer = { ...creds.payoneer, ...json.payoneer };
			if (json.payoneer_standard && typeof json.payoneer_standard === "object")
				creds.payoneer_standard = {
					...creds.payoneer_standard,
					...json.payoneer_standard,
				};
			if (json.crypto && typeof json.crypto === "object")
				creds.crypto = { ...creds.crypto, ...json.crypto };
			if (json.cryptobox && typeof json.cryptobox === "object")
				creds.cryptobox = { ...creds.cryptobox, ...json.cryptobox };
			if (json.googlepay && typeof json.googlepay === "object")
				creds.googlepay = { ...creds.googlepay, ...json.googlepay };
			if (json.attijari_morocco && typeof json.attijari_morocco === "object")
				creds.attijari_morocco = {
					...creds.attijari_morocco,
					...json.attijari_morocco,
				};
			if (json.chimoney && typeof json.chimoney === "object")
				creds.chimoney = { ...creds.chimoney, ...json.chimoney };
			if (json.xe && typeof json.xe === "object")
				creds.xe = { ...creds.xe, ...json.xe };
			if (json.wise && typeof json.wise === "object")
				creds.wise = { ...creds.wise, ...json.wise };
			if (json.transfi && typeof json.transfi === "object")
				creds.transfi = { ...creds.transfi, ...json.transfi };
		}
	...
	static missingCredentials(route, cfg) {
		...
		if (r === "wise") {
			...
			if (
				String(process.env.WISE_ENVIRONMENT || "sandbox").toLowerCase() !==
				"live"
			)
				return true;
			return false;
		...
		if (r === "paypal") {
			...
			if (isPlaceholder(c.clientId) || isPlaceholder(c.clientSecret))
				return true;
			return false;
		}
		if (r === "bank_transfer") {
	...
	static getOwnerAccountForType(type) {
		...
		if (t === "googlepay") {
			return (
				...
				normEmail(process.env.GOOGLEPAY_ACCOUNT_EMAIL)
			);
		}
		return null;
	}
