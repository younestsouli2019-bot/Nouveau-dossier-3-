...
function getClipboardContent() {
	try {
		...
		const res = spawnSync("powershell", ["-command", "Get-Clipboard"], {
			encoding: "utf8",
		});
...
async function main() {
	...
	try {
		...
		if (clip && clip.length > 5) {
			...
			const useClip = await question(
				"Is this the SMS content? (Y/n) [Default: Y]: ",
			);
		...
		if (!smsContent) {
			...
			console.log(
				"Please COPY the SMS to your clipboard and press Enter, OR paste it below.",
			);
			smsContent = await question("SMS Content: ");
		}

		if (!smsContent.trim())
			throw new Error("No content acquired. Cannot proceed.");
		...
		if (
			smsContent.match(/code|activation|mot de passe/i) &&
			!smsContent.match(/virement|reçu|compte/i)
		) {
			isActivation = true;
		...
		if (isActivation) {
			console.log(
				"This appears to be an access credential for 'attijaripaypal.attijariwafa.com'.",
			);
			console.log(
				"It will be logged as a 'Security Event' rather than Revenue.",
			);
			...
			if (amount === 0) {
				const amountStr = await question(
					"2. Could not auto-detect Amount. Enter Amount (e.g. 1000.00): ",
				);
				amount = parseFloat(amountStr);
			}
			if (!currency || currency === "MAD") {
				// Ask to confirm if default
				const curInput = await question(
					`3. Currency detected: ${currency}. Press Enter to confirm or type correct (USD/EUR): `,
				);
		...
		const dateStr = await question(
			"4. Enter Date (YYYY-MM-DD) [Default: Today]: ",
		);
		const date = dateStr
		...
		const sender =
			(await question(
				"5. SMS Sender (e.g. Attijari, 2323) [Default: Attijari]: ",
			)) || "Attijari";

		// Proof ID
		const proofHash = sha256(
			`${sender}:${date}:${amount}:${currency}:${smsContent.trim()}`,
		).slice(0, 16);
		...
		if (!isActivation && (currency === "USD" || currency === "EUR")) {
			...
			console.log(
				`> Retain (Compte Devises): ${split.retained.amount} ${currency} (70%)`,
			);
			console.log(
				`> Convert (Compte Dirham): ${split.converted.amount} ${currency} -> ~${(split.converted.amount * 10).toFixed(2)} MAD (30%)`,
			);
		...
		console.log(
			`Type: ${isActivation ? "ACCESS_CREDENTIAL" : "REVENUE_DEPOSIT"}`,
		);
		...
		const result = await createBase44RevenueEventIdempotent(
			base44,
			cfg,
			event,
			{ dryRun: false },
