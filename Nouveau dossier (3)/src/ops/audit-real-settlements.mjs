...
async function auditSettlements() {
	...
	const payoneerFiles = await listFiles(
		"settlements/payoneer/historical",
		".csv",
	);
	...
	const summary = {
		...
		real_cash_confirmed: evidence.payoneer.reduce(
			(sum, i) => sum + i.amount,
			0,
		), // Only Payoneer seems confirmed
		pending_instructions: evidence.bank_wires.reduce(
			(sum, i) => sum + i.amount,
			0,
		),
	};
