...
export async function runHunterGathererLoop() {
	...
	for (const signal of freshSignals) {
		console.log(
			`[Gatherer] Processing Signal: "${signal.topic}" (Confidence: ${signal.confidence})`,
		);
		...
		try {
			...
			console.log(
				`[Gatherer] SUCCESS: Production started for "${signal.topic}"`,
			);
		} catch (e) {
			console.error(
				`[Gatherer] FAILED: Could not process "${signal.topic}" - ${e.message}`,
			);
		}
	}
