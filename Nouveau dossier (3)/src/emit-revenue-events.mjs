...
async function submitPayPalPayoutBatch(base44, { batchId, args, dryRun }) {
	...
	for (const it of items) {
		const senderItemId = payoutItemCfg.fieldMap.itemId
			...
			: null;
		let recipient = payoutItemCfg.fieldMap.recipient
		...
		if (!valid)
			throw new Error(
				`CRITICAL: Attempted payout to unauthorized destination: ${resolved}`,
			);
	...
	for await (const line of rl) {
		...
		if (!row.id || String(row.id).trim() === "") {
			process.stderr.write(
				`Skipping row with missing ID: ${JSON.stringify(row)}\n`,
			);
			continue;
		}
...
async function main() {
	...
	if (!externalIdRaw) {
		throw new Error(
			"STRICT REVENUE GATING: --external-id is REQUIRED. Simulation/Manual IDs are FORBIDDEN.",
		);
	}
