...
function buildRevenueData(cfg, event) {
	...
	if (
		...
		throw new Error(
			"REVENUE SECURITY: Simulation sources are PERMANENTLY BANNED.",
		);
	}
	if (eid.includes("mock") || eid.includes("sim_") || eid.includes("demo_")) {
		throw new Error(
			"REVENUE SECURITY: Mock/Demo External IDs are PERMANENTLY BANNED.",
		);
	}
	if (!event.externalId || String(event.externalId).trim() === "") {
		throw new Error(
			"REVENUE SECURITY: External ID is REQUIRED for all revenue events.",
		);
	}
