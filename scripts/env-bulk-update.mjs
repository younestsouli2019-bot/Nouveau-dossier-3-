...
function upsertEnv(lines, name, value) {
  ...
  // Insert before non-env section to keep env block at top
  let insertAt = 0;
  while (insertAt < lines.length && /^\s*\$env\s*:/.test(lines[insertAt])) insertAt++;
...
function parseDotEnv(filePath) {
  ...
  try {
    ...
    for (const line of txt.split(/\r?\n/)) {
      ...
      if (!v || /^<\s*YOUR_[A-Z0-9_]+\s*>$/.test(v) || /^YOUR_[A-Z0-9_]+$/.test(v) || /^(REPLACE_ME|CHANGEME|TODO)$/.test(v)) continue;
      out[k] = v;
    }
  } catch {}
  return out;
...
function main() {
  ...
  if (args["enable-all"] || args.enableAll === true) {
    ...
    // DO NOT auto-enable PPP2 send flags; requires owner approval and compliance
  }

  saveLines(credsPath, lines);
  process.stdout.write(`${JSON.stringify({ ok: true, file: credsPath, lines: lines.length }, null, 2)}\n`);
}
