...
async function main() {
	...
	const provider = String(
		args.provider || process.env.MICRO_PROVIDER || "bybit",
	).toLowerCase();
	const amount = Number(args.amount || process.env.MICRO_AMOUNT_USDT || "3.7");
	const network = String(
		args.network || process.env.CRYPTO_NETWORK || "TON",
	).toUpperCase();
	const dest =
		args.address ||
	...
	if (!dest || !amount || !(amount > 0)) {
		console.log(
			JSON.stringify({ ok: false, error: "missing_destination_or_amount" }),
		);
		return;
	}
	...
	console.log(
		JSON.stringify({ ok: true, provider, network, dest, result: res }),
	);
}
