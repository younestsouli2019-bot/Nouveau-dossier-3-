...
function looksSimulatedId(s) {
	return typeof s === "string" && /^(sim_|pyr_sim_)/i.test(s);
}
...
function detectSimulationMarkers(j) {
	const markers = [];
	const provider = (j.provider || j.gateway || j.network || "")
		.toString()
		.toUpperCase();
	...
	if (provider === "SIMULATED")
		markers.push({ kind: "provider_simulated", reason: "provider=SIMULATED" });
	if (looksSimulatedId(payoutId))
		markers.push({ kind: "id_simulated", reason: `id=${payoutId}` });
	if (
		(provider === "BANK_WIRE" || provider === "BANK") &&
		(!j.provider || j.provider === "") &&
		status === "prepared"
	) {
		markers.push({
			kind: "bank_prepared_placeholder",
			reason: "bank wire prepared without provider ref",
		});
	}
	if (txId) {
		if (!isHex64(txId))
			markers.push({ kind: "invalid_tx_format", reason: `txId=${txId}` });
	} else if (provider === "CRYPTO") {
		markers.push({
			kind: "missing_tx",
			reason: "crypto receipt without tx id",
		});
	}
	return markers;
}
...
async function processJsonFile(file, { dryRun }) {
	try {
		...
		const existing = Array.isArray(j.verification_markers)
			? j.verification_markers
			: [];
		...
		const simulationDetected = existing.some(
			(m) =>
				m.kind.includes("simulated") || m.kind === "bank_prepared_placeholder",
		);
		const fakeTxDetected = existing.some((m) => m.kind === "invalid_tx_format");

		j.verification_markers = existing;
		if (simulationDetected && j.simulation_detected !== true)
			j.simulation_detected = true;
		if (fakeTxDetected && j.fake_txhash_detected !== true)
			j.fake_txhash_detected = true;
		...
		return {
			file,
			updated: added > 0 || simulationDetected || fakeTxDetected,
			markers: existing,
		};
	} catch {
		return { file, updated: false, markers: [], error: true };
	}
}
...
async function main() {
	...
	const outFile = path.join(
		outDir,
		`simulation_audit_report_${Date.now()}.json`,
	);
