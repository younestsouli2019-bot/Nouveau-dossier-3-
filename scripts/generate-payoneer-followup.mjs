	...
	function walk(dir) {
		...
		for (const e of entries) {
			...
			if (e.isDirectory()) {
				...
				if (name.endsWith(".txt") && name.includes(String(ref).toLowerCase()))
					out.push(p);
			}
		}
	}
...
function buildEmail({
	...
	lines.push(``);
	lines.push(
		`I am writing to inquire about a pending balance of $${amountUsd} USD from my digital services aggregator.`,
	);
	lines.push(
		`The system indicates these funds are currently "Held by Platform" in my Global Payment Service receiving account.`,
	);
	lines.push(``);
	lines.push(`Details:`);
	...
	lines.push(
		`- Source: Swarm Autonomous Revenue Aggregator (Global Payment Service)`,
	);
	...
	lines.push(``);
	lines.push(
		`I have attached the Payment Remittance Advice generated by the source platform for your reference.`,
	);
	lines.push(
		`Please confirm when these funds will be released to my available balance for withdrawal.`,
	);
	lines.push(``);
...
async function main() {
	...
	const batchRef =
		String(args.batch || args.batch_ref || args.ref || "").trim() ||
		"BATCH_PAYONEER_X_1767529200";
	const amountUsd = String(
		args.amount || process.env.SETTLEMENT_AMOUNT_USD || "1200",
	).trim();
	...
	const purpose =
		process.env.SETTLEMENT_PURPOSE || "Digital Software Development Services";
	...
	const jsonPath = path.join(
		outDir,
		`payoneer_followup_${batchRef}_${ts}.json`,
	);
	...
	console.log(
		JSON.stringify(
			{ ok: true, txt: txtPath, json: jsonPath, attachment: attachmentPath },
			null,
			2,
		),
	);
