...
import { getPayoutBatchConfigFromEnv, resolveRecipientAddress, normalizeRecipientType } from "../src/emit-revenue-events.mjs";
...
async function main() {
  ...
  const batches = await batchEntity.filter({ [payoutBatchCfg.fieldMap.status]: "pending_approval" }, "-created_date", 100, 0);
  ...
  for (const batch of batches) {
    ...
    console.log(`Checking batch ${batchId} (Type: ${recipientType}, Beneficiary: ${beneficiary})`);

    // Determine owner account for this type
    const ownerAccount = OwnerSettlementEnforcer.getOwnerAccountForType(recipientType);
    ...
    if (noteRecipient && ownerAccount) {
      if (String(noteRecipient).toLowerCase() === String(ownerAccount).toLowerCase()) {
        isOwner = true;
      }
    }
    ...
    if (!isOwner && beneficiary) {
       ...
       if (ownerName && String(beneficiary).toLowerCase().includes(String(ownerName).toLowerCase())) {
         isOwner = true;
       }
    ...
    if (isOwner) {
      ...
      console.log(`  -> Not identified as owner (Owner Account: ${ownerAccount}, Batch Recipient: ${noteRecipient}). Skipping.`);
    }
  }
}
