...
class ConstitutionalGuard {
  static validateRevenueSettlement(events, settlementAmount) {
    ...
    // Check total amount
    if (settlementAmount > CONSTITUTION.financial_constraints.max_settlement_per_transaction_usd) {
      violations.push({
...
class HistoricalRevenueProcessor {
  constructor() {
    this.revenueCsvPath = path.join(process.cwd(), 'RevenueEvent_export (1).materialized.csv');
    this.outputDir = path.join(process.cwd(), 'settlements', 'payoneer', 'historical');
    this.ownerConfig = {
      recipient_email: process.env.OWNER_PAYONEER_EMAIL || process.env.SETTLEMENT_REQUESTOR_EMAIL || 'younestsouli2019@gmail.com', // Fallback to known owner
      recipient_name: process.env.OWNER_NAME || process.env.SETTLEMENT_REQUESTOR_NAME || 'Younes Tsouli',
  ...
  filterUnsettledEvents(events) {
    const unsettledEvents = events.filter(event => {
      ...
      const isEarned = status === 'earned' || status === 'confirmed' || status === 'cleared';
      const hasBatch = event.payout_batch_id && event.payout_batch_id.trim().length > 0;
      return isEarned && !hasBatch && event.is_sample === false && parseFloat(event.amount) > 0;
    });
  ...
  async generatePayoneerBatch(events, summary, batchIndex = 0) {
    ...
    const manifest = {
      settlement_batch: {
        ...
        constitutional_validation: ConstitutionalGuard.validateRevenueSettlement(
          events, 
    ...
    const csvContent = stringify(csvRows, {
      header: true,
      columns: [
        'recipient', 'recipient_email', 'recipient_name', 'amount', 'currency',
        'batch_id', 'item_id', 'note', 'payer_name', 'payer_email', 'payer_company',
        'purpose', 'reference', 'prq_link'
      ]
    });
    ...
    return { manifest, csvPath, batchId, totalAmount: Object.values(summary)
  ...
  async createExcelVersion(csvRows, batchId) {
    try {
      ...
      console.log('⚠️  Could not create Excel file (xlsx module not available), CSV only');
    }
  }
  ...
  async updateLedger(batchId, events) {
    ...
    const updateRecord = {
      ...
      constitutional_check: ConstitutionalGuard.validateRevenueSettlement(events, 
...
async function settleHistoricalRevenues() {
  ...
  try {
    ...
    console.log(`📦 Split ${unsettledEvents.length} events into ${chunks.length} batches to satisfy limit (<$${MAX_BATCH_AMOUNT})`);
    ...
    for (const chunk of chunks) {
      ...
      const totalAmount = Object.values(summary).reduce((sum, data) => sum + data.total, 0);

      console.log(`\nProcessing Batch ${batchIndex}/${chunks.length} ($${totalAmount.toFixed(2)})...`);

      // Constitutional validation per chunk
      const validation = ConstitutionalGuard.validateRevenueSettlement(chunk, totalAmount);
      ...
      if (!validation.valid) {
        ...
        return { settled: false, reason: 'CONSTITUTION_VIOLATION', violations: validation.violations };
      }

      // Human approval check
      if (validation.requires_human_approval && !process.argv.includes('--approve')) {
         ...
         const reviewBatch = await processor.generatePayoneerBatch(chunk, summary, batchIndex);
         return {
            settled: false,
            reason: 'REQUIRES_HUMAN_APPROVAL',
            review_files: {
              csv: reviewBatch.csvPath,
              manifest: path.join(processor.outputDir, `${reviewBatch.batchId}_manifest.json`)
            },
            amount: totalAmount,
      ...
      // Generate files
      const result = await processor.generatePayoneerBatch(chunk, summary, batchIndex);
    ...
    console.log('='.repeat(50));
    console.log(`💰 Total Settled: $${results.reduce((acc, r) => acc + r.totalAmount, 0).toFixed(2)}`);
...
if (import.meta.url === pathToFileURL(process.argv[1]).href) {
  ...
  settleHistoricalRevenues().then(result => {
    if (result.settled) {
      process.exit(0);
    } else {
...
export { settleHistoricalRevenues, HistoricalRevenueProcessor, ConstitutionalGuard };
