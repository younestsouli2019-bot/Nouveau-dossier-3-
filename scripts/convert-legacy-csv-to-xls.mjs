...
function buildXls({ headers, rows, sheetName = "Legacy" }) {
	...
	const headerRow = headers
		...
		.join("");
	const dataRows = rows
		.map((r) => `<Row>${r.map((v, i) => asCell(v, headers[i])).join("")}</Row>`)
		.join("");
...
function convertFile(csvPath, opts = {}) {
	...
	const colIndex = Object.fromEntries(
		headers.map((h, i) => [String(h).trim(), i]),
	);
	...
	const payerEmailDefault =
		...
		"";
	const payerNameDefault =
		opts.payerName || process.env.SETTLEMENT_REQUESTOR_NAME || "";
	const payerCompanyDefault =
		opts.payerCompany || process.env.SETTLEMENT_REQUESTOR_COMPANY || "";
	const purposeDefault = opts.purpose || process.env.SETTLEMENT_PURPOSE || "";
	const referenceDefault =
		opts.reference || process.env.SETTLEMENT_REFERENCE || "";
	const noteDefault = opts.note || process.env.SETTLEMENT_NOTE || "";
	const currencyDefault =
		opts.currency || process.env.SETTLEMENT_CURRENCY || "USD";
	const filteredRows = rows.filter((r) => {
		const idxRecipient = colIndex["recipient"];
		const idxAmount = colIndex["amount"];
		const hasRecipient =
			idxRecipient != null && String(r[idxRecipient] || "").trim() !== "";
		const hasAmount =
			idxAmount != null && String(r[idxAmount] || "").trim() !== "";
		...
		return finalHeaders.map((h) => {
			if (!headerSet.has(h)) {
				if (h === "recipient_email")
					return get("recipient")?.includes("@") ? get("recipient") : "";
