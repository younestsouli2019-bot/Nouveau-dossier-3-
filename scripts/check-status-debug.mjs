...
const approvedBatches = batches.filter(b => b.status === 'approved' || b.status === 'pending_approval');
...
approvedBatches.forEach(b => {
    console.log(`ID: ${b.batch_id} | Amount: ${b.total_amount} ${b.currency} | Status: ${b.status} | Created: ${b.created_at || 'N/A'}`);
});
