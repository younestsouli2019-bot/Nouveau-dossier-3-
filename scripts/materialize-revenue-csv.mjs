...
function resolveInputOutput(args) {
	...
	const input = arg("--in", path.join(cwd, "RevenueEvent_export (1).csv"));
	const output = arg(
		"--out",
...
function main() {
	const { input, output, overwrite } = resolveInputOutput(
	...
	const { rows: updatedRows, updatedCount } = materializeProjectedToEarned(
		headers,
		rows,
	);
	if (updatedCount === 0) {
