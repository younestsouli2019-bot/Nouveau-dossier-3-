...
function buildCheckoutStub(title, body) {
	return `<!doctype html><html><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>${title}</title><style>body{font-family:system-ui,-apple-system,Segoe UI,Roboto,Ubuntu,Cantarell,Noto Sans,sans-serif;margin:20px}header{margin-bottom:20px}</style></head><body><header><h1>${title}</h1><nav><a href="/catalog/index.html">Back to Catalog</a></nav></header><p>${body}</p></body></html>`;
}
...
function buildBundlePage(title, items) {
	const cards = items
		.map((c) => {
			const hue = Math.abs(hashCode(c.slug)) % 360;
			return `<a class="card" href="/catalog/${c.slug}.html"><div class="thumb"><img alt="${c.title}" src="data:image/svg+xml;charset=utf-8,${encodeURIComponent(`<svg xmlns='http://www.w3.org/2000/svg' width='560' height='240'><defs><linearGradient id='g' x1='0' y1='0' x2='1' y2='1'><stop offset='0%' stop-color='hsl(${hue},72%,52%)'/><stop offset='100%' stop-color='hsl(${(hue + 40) % 360},68%,46%)'/></linearGradient></defs><rect width='100%' height='100%' rx='12' fill='url(#g)'/><text x='50%' y='50%' dominant-baseline='middle' text-anchor='middle' fill='white' font-family='system-ui' font-size='20'>${String(c.title || "").slice(0, 42)}</text></svg>`)}"/></div><div class="content"><div class="title">${c.title}</div><div class="meta"><span class="pill">Practice ${c.practiceTestCount}</span><span class="pill">Lectures ${c.lectureCount}</span><span class="pill">Quizzes ${c.quizCount}</span></div><div class="cta">Enroll</div></div></a>`;
		})
		.join("");
	return `<!doctype html><html><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><meta http-equiv="Content-Security-Policy" content="default-src 'self'; img-src 'self' data:; style-src 'self' 'unsafe-inline'; script-src 'self'; connect-src 'self'; frame-ancestors 'none'; base-uri 'none'"><title>${title}</title><style>body{margin:0;font-family:system-ui,-apple-system,Segoe UI,Roboto,Ubuntu,Cantarell,Noto Sans,sans-serif;background:#0c0e12;color:#eaeef2}header{padding:28px 20px;background:radial-gradient(1200px 500px at 20% -100%,#1a1f3b 0%,transparent 70%)}.wrap{max-width:1200px;margin:0 auto}.grid{display:grid;grid-template-columns:repeat(auto-fill,minmax(280px,1fr));gap:16px;padding:20px}.card{display:grid;grid-template-rows:160px auto;background:#0e1118;border:1px solid #1e2532;border-radius:12px;overflow:hidden;text-decoration:none;color:inherit}.thumb{background:#222}.content{padding:12px}.title{font-size:16px;font-weight:600;color:#f2f6ff;line-height:1.3;margin-bottom:8px}.meta{display:flex;flex-wrap:wrap;gap:6px;margin-bottom:10px}.pill{display:inline-flex;padding:4px 8px;border-radius:999px;background:#151924;border:1px solid #2a3344;color:#cbd3df;font-size:12px}.cta{display:inline-flex;align-items:center;padding:8px 12px;border-radius:8px;background:#16263a;color:#eaf1ff;border:1px solid #2a3e58}</style></head><body><script>(function(){try{if(location.search){history.replaceState(null,"",location.origin+location.pathname+location.hash);}}catch(e){}})();</script><header><div class="wrap"><h1>${title}</h1><div style="margin-top:10px"><a class="cta" href="/checkout/paypal.html">Checkout</a></div></div></header><main><div class="wrap"><div class="grid">${cards}</div></div></main></body></html>`;
}
...
function main() {
	...
	const checkoutDir = path.join(outStatic, "checkout");
	ensureDir(checkoutDir);
	writeText(
		path.join(checkoutDir, "paypal.html"),
		buildCheckoutStub(
			"PayPal Checkout",
			"Proceed to secure checkout. Integration activates in live backend.",
		),
	);
	writeText(
		path.join(checkoutDir, "payoneer.html"),
		buildCheckoutStub(
			"Payoneer Checkout",
			"Proceed to secure checkout. Integration activates in live backend.",
		),
	...
	writeText(
		path.join(checkoutDir, "crypto.html"),
		buildCheckoutStub(
			"Crypto Checkout",
			"Proceed to secure checkout. Integration activates in live backend.",
		),
	...
	writeText(
		path.join(checkoutDir, "bank.html"),
		buildCheckoutStub(
			"Bank Wire",
			"Proceed to secure checkout. Integration activates in live backend.",
		),
