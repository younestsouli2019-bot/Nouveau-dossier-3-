...
function scanWireDirs({ occurredAtMs, expectedTotal, destRib }) {
  ...
  for (const dir of dirs) {
    ...
    const files = fs.readdirSync(dir).filter(f => f.toLowerCase().includes("wire"));
    for (const f of files) {
      try {
        ...
        const ts = Date.parse(String(data.timestamp || data.created_at || Date.now()));
        const destination = String(data.destination || data.bank_rib || "").trim();
        const amount = Number(data.amount || data.total || 0);
        const status = String(data.status || "").toLowerCase();
        const okDest = destRib ? destination.includes(destRib) : true;
        const okTime = Number.isFinite(occurredAtMs) ? ts >= occurredAtMs : true;
        const tol = 1e-2;
        const okAmount = expectedTotal > 0 ? Math.abs(amount - expectedTotal) < tol : true;
...
async function main() {
  ...
  if (!inputCsv || !fs.existsSync(inputCsv)) {
    console.log(JSON.stringify({ ok: false, error: "missing_or_invalid_input_csv", file: inputCsv }));
    process.exit(1);
    return;
  }
  ...
  const destRib = String(process.env.OWNER_BANK_RIB || "007810000448500030594182").trim();
  console.log(JSON.stringify({ watching: true, file: inputCsv, expectedTotal, destRib }));
