...
const LOG = resolve(ROOT, 'data/out/autorun-full.log');
...
try {
  const entry = `
process.argv.push('autorun-entry');
import('./scripts/auto-run-v352-suite.ts').then(async (m) => {
  try { const code = await m.main(); process.exit(code ?? 0); }
  catch (e) { console.error('ENTRY_ERROR', e?.message ?? String(e)); process.exit(98); }
